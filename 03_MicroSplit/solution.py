...
assert len(STRUCTURES) > 1, "Choose at least two structures"

# %% [markdown] tags=[]
# The following cell creates data configuration for training, validation and test sets. Each configuration defines a set of parameters related to data loading, dataset creation and data processing.
#
...
# - `earlystop_patience : int` -> number of epochs to wait before applying earlystop. Earlystop consists in stopping training process before it reaches the desired number of epochs if a monitored metric (usually the aggregated validation loss) doesn't improve over some amount of epochs.
...
#
# In this section we will train our MicroSplit model using `lightning`. We have manually set the time limit to 10 minutes. This limit can be modified with the `max_time` argument.
...
# - ***Outer padding***: the patch (tile) size used for training (e.g., `(64, 64)`) is padded to a larger size. Then, the padded area is discarded during stitching.
...
#
# In this section you will be provided with tools to interactively inspect the predicted unmixed images from the test set to have a preliminary qualitative evaluation and spot potential issues.
...
# But, if you do and you want to keep going, you have the opportunity to work with [04_bonus_COSDD](../04_bonus_COSDD/exercise.ipynb).
